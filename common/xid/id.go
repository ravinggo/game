...
var (
	...
	objectIDStringCounter atomic.Uint32
	...
	NilIDString  IDString
	baseIDString IDString
...
func init() {
	...
	baseIDString = createBaseIDString()
}
...
// NewIDString generates a globally unique IDString With ID.String()
func NewIDString() IDString {
	return NewIDStringWithTime(time.Now())
}

func NewIDStringWithTime(tm time.Time) IDString {
	t := tm.Unix()
	dst := baseIDString
	i := atomic.AddUint32(&objectIDCounter, 1)
	dst[19] = encoding[(byte(i)<<4)&0x1F]
	dst[18] = encoding[(byte(i)>>1)&0x1F]
	dst[17] = encoding[(byte(i)>>6)|(byte(i>>8)<<2)&0x1F]
	dst[16] = encoding[byte(i>>8)>>3]
	dst[15] = encoding[byte(i>>16)&0x1F]
	dst[14] = encoding[(byte(i>>16)>>5)|(byte(pid)<<3)&0x1F]
	// t := uint32(time.Now().Unix())
	dst[6] = encoding[machineID[0]>>5|(byte(t)<<3)&0x1F]
	dst[5] = encoding[(byte(t)>>2)&0x1F]
	dst[4] = encoding[byte(t)>>7|(byte(t>>8)<<1)&0x1F]
	dst[3] = encoding[(byte(t>>8)>>4)|(byte(t>>16)<<4)&0x1F]
	dst[2] = encoding[(byte(t>>16)>>1)&0x1F]
	dst[1] = encoding[(byte(t>>16)>>6)|(byte(t>>24)<<2)&0x1F]
	dst[0] = encoding[byte(t>>24)>>3]
...
func createBaseIDString() IDString {
	id := baseID()
	var dst IDString
...
func (is IDString) String() string {
	return utils.BytesToString(is[:])
}

func (is IDString) IsNil() bool {
	return is[0] == 0
}

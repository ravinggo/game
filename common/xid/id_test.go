...
func TestIDString2(t *testing.T) {
	s := NewIDString()
	id, err := FromString(s.String())
	if err != nil {
		t.Errorf("FromString() error got %v, want nil", err)
	}
	if id.String() != s.String() {
		t.Errorf("id.String() got %v, want %v", s.String(), id.String())
	}
}
